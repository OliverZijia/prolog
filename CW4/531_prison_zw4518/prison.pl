...
is_change(N,W,Index,S1,S):-		% S=1:should change; S=0:no change
	(W >= N
	-> (0 is mod(Index,N)
	   -> S2 is S1+1
	   ;  S2 is S1),
...
cell_status(Cell, N, Status) :-
	...
	;  ((N > Cell
	    -> N1 = Cell
	    ;  N1 = N),
	   changed(Cell,1,N1,0) 	   	%changed(Cell,Warder,Num_warder,Status)
	   -> Status = unlocked
	   ;  Status = locked)
	).

changed(Cell,W,N,S):-			% S=1:should change; S=0:no change
	(N >= W
	-> (0 is mod(Cell,W)
	   -> S1 is S + 1
	   ;  S1 = S),
